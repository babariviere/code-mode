...
export interface SpindleSandboxOptions {
	...
	/** Provider namespaces to install as guest proxies. */
...
export class QuickJsRuntime {
	async execute(
		...
		try {
			...
			const providers = jsonHandle(context, jsonObject, jsonParse, options.providers ?? []);
			context.setProp(context.global, "__spindleProviders", providers);
			providers.dispose();
